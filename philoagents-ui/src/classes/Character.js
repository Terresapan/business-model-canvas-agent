class Character {
  constructor(scene, config) {
    ...
    this.atlasId = config.atlasId || this.atlas; // Use atlasId if provided, otherwise use atlas
    this.defaultFrame = `${this.atlasId}-${config.defaultDirection || "front"}`;
      ...
      if (!anims.exists(animKey)) {
        anims.create({
          ...
          frames: anims.generateFrameNames(this.atlas, {
            prefix: `${this.atlasId}-${direction}-walk-`,
            end: 8,
            zeroPad: 4,
          }),
          frameRate: 10,
  ...
  facePlayer(player) {
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      this.sprite.setTexture(
        this.atlas,
        `${this.atlasId}-${dx < 0 ? "left" : "right"}`
      );
    } else {
      this.sprite.setTexture(
        this.atlas,
        `${this.atlasId}-${dy < 0 ? "back" : "front"}`
      );
    }
  }
  ...
  chooseNewDirection() {
    ...
    if (Math.random() < 0.4) {
      ...
      if (this.scene.anims.exists(animKey)) {
        ...
        this.sprite.setTexture(
          this.atlas,
          `${this.atlasId}-${this.getDirectionFromMovement()}`
        );
      }

      this.moveDuration = Phaser.Math.Between(500, 1000);
      this.movementTimer = this.scene.time.delayedCall(
        this.moveDuration,
        () => {
      ...
      const direction = ["front", "back", "left", "right"][
        Math.floor(Math.random() * 4)
      ];
      this.sprite.setTexture(this.atlas, `${this.atlasId}-${direction}`);
